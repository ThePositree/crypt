...
def _bucket_periods(
    ...
    if start is not None and end is not None:
        ...
        return pd.period_range(start=start_ts, end=end_ts, freq=freq).astype(str).tolist()
    ...
    if entry.empty:
        return []
    return pd.period_range(start=entry.min(), end=entry.max(), freq=freq).astype(str).tolist()
