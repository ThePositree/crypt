...
def make_trending_up_h4(n: int = 210) -> pd.DataFrame:
    ...
    for i, c in enumerate(prices):
        ...
        h = c * 1.003
        lo = c * 0.997
        rows.append({"open_time": t + timedelta(hours=4 * i), "o": o, "h": h, "l": lo, "c": c, "volume": 1000.0})
...
def make_sideways_h4(n: int = 210) -> pd.DataFrame:
    ...
    for i in range(n):
        ...
        rows.append({"open_time": t + timedelta(hours=4 * i), "o": c, "h": h, "l": lo, "c": c, "volume": 500.0})
    return pd.DataFrame(rows)
...
def make_oversold_h4() -> pd.DataFrame:
    ...
    for i in range(40):
        ...
        rows.append({"open_time": t + timedelta(hours=4 * i), "o": c, "h": h, "l": lo, "c": c, "volume": 1000.0})
    for i in range(20):
        c *= 0.975  # -2.5% per candle
        h, lo = c * 1.002, c * 0.998
        rows.append({"open_time": t + timedelta(hours=4 * (40 + i)), "o": c, "h": h, "l": lo, "c": c, "volume": 2000.0})
    return pd.DataFrame(rows)
...
def make_overbought_h4() -> pd.DataFrame:
    ...
    c = 100.0
    for i in range(40):
        rows.append({"open_time": t + timedelta(hours=4 * i), "o": c, "h": c * 1.002, "l": c * 0.998, "c": c, "volume": 1000.0})
    for i in range(20):
        c *= 1.025  # +2.5% per candle
        rows.append({"open_time": t + timedelta(hours=4 * (40 + i)), "o": c, "h": c * 1.002, "l": c * 0.998, "c": c, "volume": 2000.0})
    return pd.DataFrame(rows)
