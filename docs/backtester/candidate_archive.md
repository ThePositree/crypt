...
2. preserve research strategies for future regime discovery and detector
   training, even when they do not pass the strategy benchmark.
...
| Benchmark **archive** verdict (any month intra-month DD > 10%) | Archive with reason `benchmark_dd` |
...
| Benchmark **discard** with no redeeming research value | Do **not** archive; note in BACKLOG only |
...
The archive entry must state the reason. Do not hide benchmark failure. Research
...
docs/archive/candidates/
  ...
  <candidate_id>/
    ...
    mandate_snapshot.md              # benchmark summary snapshot at archive time
...
| Location | Role |
| -------- | ---- |
| `strategies/backtester/` | Active research/backtester configs |
...
- ADR-0025 — original benchmark gates
- ADR-0041 — regime discovery from archived strategy behavior
- `docs/strategy_benchmark.md` — current benchmark and owner override rule
