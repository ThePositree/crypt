...
but worth keeping (superseded, near-miss, or mandate **archive** tier per
ADR-0025).
...
| Mandate **discard** with no redeeming metrics | Do **not** archive; note in BACKLOG only |
...
docs/archive/candidates/
  ...
  <candidate_id>/
    README.md                        # owner-facing verdict (required)
...
3. **Best mandate run** (symbol, window, verdict, sum capped, months ≥15%)
4. **Best execution params** (exit geometry, tp/rrr/ttl/risk)
5. **Why not promoted** (one paragraph)
...
| `docs/archive/candidates/` | Verdict + mandate snapshots |

Do not delete archived strategy JSON from git. Do not run Optuna on archived
candidates unless the owner explicitly revives one in chat.
